...
![战役子菜单](./assets/campaign-submenu.png)
...
![战役初始舰队选择](./assets/campaign-initial-fleet-selection.png)
...
![战斗舰船职能](./assets/romani-combat-ship-roles.png)
...
![防御导弹](./assets/missile-defence.png)
...
![情报消息](./assets/intelligence-tips.png)
...
一旦您使用雷达，就有可能被敌人在一定距离上侦测到——但反之亦然。不过，您可以采取预防措施以自我保护。
...
[红外搜索与跟踪系统][IRST]侦测来自空中与地面载具的热信号。它的探测范围比雷达小得多，但是无法被敌方察觉，因此使用起来总是安全的。

![IRST](./assets/irst.png)

1. **红外扫描器** - 侦测到的红外信号会在对应方向显示为尖峰。
2. **方位旋钮** - 用鼠标滚轮旋转可选择一特定区域集中扫描。
3. **详情屏幕** - 此屏幕显示红外源的细节。
4. **截获警告** - 侦测到热源时以蓝色闪烁。

![IRST详情](./assets/irst-detail.png)

> 留意地面上的有趣事物……您永远猜不到会发现什么。
...
[IRST]: #红外搜索与跟踪系统 "Infrared Search & Track (IRST)"

Infrared Search & Track (IRST)，红外搜索与跟踪系统。
